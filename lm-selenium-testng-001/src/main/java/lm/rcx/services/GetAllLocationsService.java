	...
	public class GetAllLocationsService extends CommonMethods{
		...
		public static void main(String[] args){
			...
			try {
				new GetAllLocationsService().runGetAllLocationsService();
			} catch (Exception e) {
		...
		public boolean runGetAllLocationsService() throws Exception {
			boolean flag = false;
			boolean flag1=false;

			WsdlProject project = getProject(System.getProperty("user.dir") + "/GetAllLocations-soapui-project.xml");

			try {
				
				reportInfo("Checking for existing Locations through services.");
				
				List<TestSuite> testSuiteList = project.getTestSuiteList();

				for (TestSuite ts : testSuiteList) {

					String suiteName = "GetAllLocations";
					if (ts.getName().equalsIgnoreCase(suiteName)) {
						...
						for (TestCase testcase : testCaseList) {
							...
							for (TestStepResult tcr : testRunner.getResults()) {
								...
								if(tcr.getTestStep().getName().equalsIgnoreCase("GetAllLocations")){
							...
							if (TestCaseRunner.Status.FINISHED == testRunner.getStatus()) {
								...
								reportInfo("All Locations retrieved successfully through services.");
							}
