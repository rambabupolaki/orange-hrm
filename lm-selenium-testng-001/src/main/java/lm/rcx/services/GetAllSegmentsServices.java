	...
	public class GetAllSegmentsServices extends CommonMethods{
		...
		public static void main(String[] args){
			...
			try {
				new GetAllSegmentsServices().runGetAllSegmentsService();
			} catch (Exception e) {
		...
		public boolean runGetAllSegmentsService() throws Exception {
			boolean flag = false;
			boolean flag1=false;

			WsdlProject project = getProject(System.getProperty("user.dir") + "/GetAllSegments-soapui-project.xml");

			try {
				
				reportInfo("Checking for existing segments through services.");
				
				List<TestSuite> testSuiteList = project.getTestSuiteList();

				for (TestSuite ts : testSuiteList) {

					String suiteName = "GetAllSegments";
					if (ts.getName().equalsIgnoreCase(suiteName)) {
						...
						for (TestCase testcase : testCaseList) {
							...
							for (TestStepResult tcr : testRunner.getResults()) {
								...
								if(tcr.getTestStep().getName().equalsIgnoreCase("GetAllSegments")){
							...
							if (TestCaseRunner.Status.FINISHED == testRunner.getStatus()) {
								...
								reportInfo("All segments retrieved successfully through services.");
							}
