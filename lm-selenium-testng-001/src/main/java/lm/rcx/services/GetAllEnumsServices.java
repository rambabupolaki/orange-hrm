	...
	public class GetAllEnumsServices extends CommonMethods{
		...
		public static void main(String[] args){
			...
			try {
				new GetAllEnumsServices().runGetAllEnumsServices();
			} catch (Exception e) {
		...
		public boolean runGetAllEnumsServices() throws Exception {
			boolean flag = false;
			boolean flag1=false;

			WsdlProject project = getProject(System.getProperty("user.dir") + "/GetAllEnums-soapui-project.xml");

			try {
				
				reportInfo("Checking for existing Enums through services.");
				
				List<TestSuite> testSuiteList = project.getTestSuiteList();

				for (TestSuite ts : testSuiteList) {

					String suiteName = "GetAllEnums";
					if (ts.getName().equalsIgnoreCase(suiteName)) {
						...
						for (TestCase testcase : testCaseList) {
							...
							for (TestStepResult tcr : testRunner.getResults()) {
								...
								if(tcr.getTestStep().getName().equalsIgnoreCase("GetAllEnums")){
							...
							if (TestCaseRunner.Status.FINISHED == testRunner.getStatus()) {
								...
								reportInfo("All Enums retrieved successfully through services.");
							}
