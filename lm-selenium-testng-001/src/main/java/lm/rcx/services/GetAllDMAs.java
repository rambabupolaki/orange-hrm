...
public class GetAllDMAs extends CommonMethods{
	public static void main(String[] args) {

		try {
			new GetAllDMAs().runGetAllDMAsService();
		} catch (Exception e) {
	...
	public boolean runGetAllDMAsService() throws Exception {
		boolean flag = false;
		boolean flag1=false;

		WsdlProject project = getProject(System.getProperty("user.dir") + "/GetAllDMAs-soapui-project.xml");

		try {
			reportInfo("Checking for existing DMAs through services.");
			
			List<TestSuite> testSuiteList = project.getTestSuiteList();

			for (TestSuite ts : testSuiteList) {

				String suiteName = "GetAllDMAs";
				if (ts.getName().equalsIgnoreCase(suiteName)) {
					...
					for (TestCase testcase : testCaseList) {
						...
						for (TestStepResult tcr : testRunner.getResults()) {
							...
							if(tcr.getTestStep().getName().equalsIgnoreCase("GetAllDMAs")){
						...
						if (TestCaseRunner.Status.FINISHED == testRunner.getStatus()) {
							...
							reportInfo("All DMAs retrieved successfully through services.");
						}
